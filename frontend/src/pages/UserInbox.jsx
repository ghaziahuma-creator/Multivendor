...
const UserChatBox = ({
  ...
  return (
    <div className="w-[100%] min-h-full flex flex-col justify-between">
      ...
      <div className="px-3 h-[70vh] py-3 overflow-y-scroll">
        {messages &&
          messages.map((item, index) => (
            <div
              className={`flex w-full my-2 ${item?.sender === userId ? "justify-end" : "justify-start"}`}
            >
              {item.sender !== userId && (
                <img
                  src={`${userData?.avatar?.url}`}
              ...
              {item.text !== "" && (
                <div>
                  <div
                    className={`w-max p-2 rounded ${item.sender !== userId ? "bg-[#38c776]" : "bg-black"}  text-[#fff] h-min mr-2`}
                  >
                    <p>{item.text}</p>
                  </div>
